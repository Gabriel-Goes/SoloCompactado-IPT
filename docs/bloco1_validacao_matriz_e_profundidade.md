...
## O que foi feito
1. Ajuste do protótipo para coluna padrão de `10 m` (configurável).
...
A síntese de referências (Embrapa + artigos científicos) e a justificativa técnica do domínio de 10 m estão documentadas em:
- `sphinx/bloco1_validacao.rst`

## Observação
A adoção de 10 m é tratada como limite computacional conservador. Para interpretação agronômica nesta fase, a priorização permanece nas camadas rasas e intermediárias.
